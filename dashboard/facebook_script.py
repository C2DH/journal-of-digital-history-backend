...
def fb_post_feed(page_id: str, token: str, msg: str, link: str=None, img: bytes=None) -> str:
    payload = {'message': msg}
    if link:
        payload['link'] = link
    if img:
    ...
    r = requests.post(f"{GRAPH}/{page_id}/feed", params={'access_token': token}, data=payload)
    r.raise_for_status()
    return r.json()['id']

def fb_post_comment(post_id: str, token: str, msg: str) -> str:
    r = requests.post(f"{GRAPH}/{post_id}/comments", params={'access_token': token}, data={'message': msg})
    r.raise_for_status()
    return r.json()['id']
...
def main(ext_args: object = None):
    ...
    for i, t in enumerate(times):
        timeline.append((t, post_thread_item, [client_info, thread_texts[i], args.link, img_bytes, i], f"Thread {i+1}"))
    ...
    if independent_texts:
        ...
        for j, t in enumerate(times2):
            timeline.append((t, post_independent, [client_info, independent_texts[j], args.link, img_bytes, j], f"Independent {j+1}"))
    # sorting by time
    ...
    for seq, (rt, func, fargs, label) in enumerate(timeline):
        ...
        if run_at <= now:
            print(f"Time {run_at.isoformat()} for {label} passed; executing immediately")
            func(*fargs)
        else:
