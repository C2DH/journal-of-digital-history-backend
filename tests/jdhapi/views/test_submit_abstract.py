...
class SubmitAbstractTestCase(APITestCase):
    def setUp(self):
        ...
        self.valid_payload = {
            "title": "Test Abstract",
            "abstract": "The quick brown fox jumps over the lazy dog, demonstrating the versatility of the English language in concise communication.",
            "contact": [
                {
                    "firstname": "John",
                    "lastname": "Doe",
    ...
    def test_submit_abstract_invalid_payload_missing_github_id(self):   
        """Test submitting an abstract with an invalid payload (missing GitHub ID)."""
        url = reverse('submit-abstract')
        c=Client()
        response = c.post(
            url,
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('error', response.data)
        self.assertEqual(response.data['message'], 'At least one author must have a GitHub ID.')
        self.assertEqual(Abstract.objects.count(), 0)
        self.assertEqual(Author.objects.count(),0)
        self.assertEqual(Dataset.objects.count(), 0)
    ...
    def test_update_existing_author(self):
        ...
        updated_author = Author.objects.get(orcid="https://orcid.org/0000-0000-0000-0000")
