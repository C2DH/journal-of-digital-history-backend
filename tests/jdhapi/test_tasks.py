...
from jdhapi.tasks import get_github_issue_url_for_all_articles
...
class TestGetGithubIssueUrlForArticle(TestCase):
    def setUp(self):
        ...
        self.issue = Issue.objects.create(
            name="i", publication_date="2024-10-21T14:37:06+02:00"
        )
    ...
    def test_sets_github_issue_on_match(self, mock_get):
        mock_get.side_effect = [
            Mock(
                ...
                json=lambda: [
                    {
                        "body": f"fixes {self.abstract.pid}",
                        "html_url": "https://github.com/x/issues/1",
                    }
                ],
            ),
            Mock(status_code=200, json=list),
        ]
        get_github_issue_url_for_all_articles()
