...
@api_view(["GET"])
@permission_classes([IsAdminUser])
def get_peer_review_article_details(_):
    """
    GET /api/articles/ojs/submissions/peer-review/details

    Get the list of all articles details in peer review by stage according to their round (R1, R1, R3+) from OJS.
    For each article, the detail would be :
    - pid
    - title
    - authors
    - url to go to active OJS page
    - substatus of the per review from OJS
    ...
    try:
        with transaction.atomic():
                ...
                for field, fieldname in required_fields.items():
                    if not getattr(author, field):
                        ...
                        if fieldname == "country":
                            country = get_country_with_ROR(
                                affiliation_name=author.affiliation
                            )
                            if not country:
            ...
            if not article.data.get("title"):
                error_msg = (
                    "Field 'title' is missing in the 'data' field of the article."
                )
                logger.error(error_msg)
                raise ValidationError(error_msg)
            ...
            try:
                ...
                res = upload_manuscript_to_ojs(
                    article.abstract.pid, submission_id, pdf_file
                )
                ...
                res = assign_primary_contact_and_metadata(
